  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", padding: "16px 24px 28px", gap: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", background: "#EEF6F1", borderRadius: 18, padding: 4, gap: 4 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        ...
        <StatTile label="Best streak" value={`${bestStreak}d`} sub="Read 20 pages" color="#9B82B8" bg="#F2EEF7" />
      ...
      <div style={{ background: "#FFFFFF", borderRadius: 22, padding: "18px 18px 14px", border: "1.5px solid #EDE8E0" }}>
        <p style={{ fontFamily: "var(--font-body)", fontSize: 11, fontWeight: 700, color: "#7A9080", letterSpacing: "0.05em", textTransform: "uppercase", margin: "0 0 14px" }}>
          Habits per day
        </p>
          ...
          {WEEK_DATA.map((count, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontFamily: "var(--font-body)", fontSize: 10, fontWeight: 600, color: today ? "#4D9467" : "#7A9080" }}>
                  {WEEK_DAYS[i]}
                </span>
              </div>
            );
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          {MOOD_WEEK.map((m, i) => (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              ...
              <span style={{ fontFamily: "var(--font-body)", fontSize: 10, color: "#7A9080" }}>{WEEK_DAYS[i]}</span>
            </div>
          ))}
        </div>
      </div>
            ...
            <div key={h.name} style={{ background: "#FFFFFF", borderRadius: 18, padding: "14px 16px", border: "1.5px solid #EDE8E0" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontFamily: "var(--font-body)", fontSize: 11, color: "#7A9080", margin: "2px 0 0" }}>
                    🔥 {h.streak} day streak
                  </p>
                </div>
...
function StatTile({ label, value, sub, color, bg }: { label: string; value: string; sub: string; color: string; bg: string }) {
  return (
    <div style={{ background: bg, borderRadius: 18, padding: "12px 12px 10px", display: "flex", flexDirection: "column", gap: 3 }}>
      <p style={{ fontFamily: "var(--font-body)", fontSize: 10, fontWeight: 700, color: "#7A9080", margin: 0, textTransform: "uppercase", letterSpacing: "0.04em" }}>
        {label}
      </p>
      <p style={{ fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 700, color, margin: 0, lineHeight: 1 }}>
        {value}
      </p>
      <p style={{ fontFamily: "var(--font-body)", fontSize: 10, color: "#7A9080", margin: 0, lineHeight: 1.3 }}>
        {sub}
      </p>
    </div>
  );
