...
const MOODS = [
  { value: 1, emoji: "😔", label: "Rough",  color: "#E8A0A0", bg: "#FCEEED" },
  { value: 2, emoji: "😕", label: "Meh",    color: "#E8C88A", bg: "#FDF7EA" },
  { value: 3, emoji: "😐", label: "Okay",   color: "#A9C4E8", bg: "#EAF2FB" },
  { value: 4, emoji: "🙂", label: "Good",   color: "#6BAE82", bg: "#EEF6F1" },
  { value: 5, emoji: "😄", label: "Great",  color: "#E8975A", bg: "#FBF0E6" },
];
  ...
  if (step === "done") {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", padding: "0 32px", gap: 24, textAlign: "center" }}>
        <div style={{ fontSize: 72, animation: "bounceIn 500ms cubic-bezier(0.4,0,0.2,1) forwards" }}>🌸</div>
        <div>
          <h2 style={{ fontFamily: "var(--font-display)", fontSize: 24, fontWeight: 600, color: "#283328", margin: "0 0 8px" }}>
            Check-in complete
          </h2>
          <p style={{ fontFamily: "var(--font-body)", fontSize: 14, color: "#7A9080", lineHeight: 1.6, margin: 0 }}>
        ...
        <div style={{ background: "#EEF6F1", borderRadius: 20, padding: 16, width: "100%", textAlign: "left" }}>
          <p style={{ fontFamily: "var(--font-body)", fontSize: 11, fontWeight: 700, color: "#7A9080", margin: "0 0 6px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
            Today's mood
          </p>
          ...
          {reflection && (
            <>
              <p style={{ fontFamily: "var(--font-body)", fontSize: 11, fontWeight: 700, color: "#7A9080", margin: "12px 0 4px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                Reflection
              </p>
        ...
        <button onClick={reset} style={{ fontFamily: "var(--font-body)", fontSize: 13, color: "#7A9080", background: "none", border: "none", cursor: "pointer", textDecoration: "underline" }}>
          Edit check-in
        </button>
  ...
  if (step === "reflect") {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "16px 24px 24px" }}>
        <button onClick={() => setStep("mood")} style={{ display: "flex", alignItems: "center", gap: 6, fontFamily: "var(--font-body)", fontSize: 13, color: "#7A9080", background: "none", border: "none", cursor: "pointer", marginBottom: 20, padding: 0 }}>
          <ChevronLeft /> Back
        </button>
        <h1 style={{ fontFamily: "var(--font-display)", fontSize: 24, fontWeight: 600, color: "#283328", margin: "0 0 8px" }}>
          A little reflection
        </h1>
        <p style={{ fontFamily: "var(--font-body)", fontSize: 13, color: "#7A9080", margin: "0 0 20px", lineHeight: 1.6 }}>
        ...
        <div style={{ borderRadius: 18, padding: "14px 16px", background: mood.bg, border: `1.5px solid ${mood.color}44`, display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <p style={{ fontFamily: "var(--font-body)", fontSize: 11, fontWeight: 700, color: "#7A9080", margin: 0, textTransform: "uppercase", letterSpacing: "0.04em" }}>Feeling</p>
        ...
        <button
          ...
          style={{ width: "100%", background: "#4D9467", color: "white", borderRadius: 18, padding: "16px", border: "none", fontFamily: "var(--font-display)", fontSize: 16, fontWeight: 600, cursor: "pointer", marginBottom: 10 }}
        >
          Done ✓
        </button>
        <button
          onClick={() => { setStep("done"); onComplete(); }}
          style={{ width: "100%", background: "none", border: "none", fontFamily: "var(--font-body)", fontSize: 13, color: "#7A9080", cursor: "pointer", padding: 8 }}
        >
          Skip reflection
        </button>
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "16px 24px 24px" }}>
      <div style={{ marginBottom: 8 }}>
        <p style={{ fontFamily: "var(--font-body)", fontSize: 13, fontWeight: 600, color: "#7A9080", margin: "0 0 4px" }}>
          Daily check-in
        </p>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 12, marginBottom: 28 }}>
        {MOODS.map((mood) => (
          ...
          >
            ...
            <span style={{ fontFamily: "var(--font-body)", fontSize: 10, fontWeight: 600, color: "#7A9080" }}>
              {mood.label}
            </span>
          </button>
        ))}
      ...
      <button
        ...
        style={{
          ...
          color: selected !== null ? "white" : "#B0BDB2",
